...
import MerchCardAdmin from '../components/MerchCardAdmin';
...
const SocietyAdmin = () => {
  ...
  const handleAddMerchInitiate = () => {
    if (addMerch) {
      handleAddMerch();
    }
    setAddMerch(!addMerch);
  };
  ...
  const renderHeader = () => (
    <>
      ...
      />
      <IconButton
          icon={() => <Ionicons name={isEditable ? "checkmark" : "pencil"} size={24} color="#fff" />}
          size={24}
  ...
  const renderPosts = () => (
    <View style={styles.postsContainer}>
      <Button mode="contained" style={[styles.addButton, { marginBottom: 10 }]} onPress={() => handleAddPostInitiate()}>Add Post</Button>  
  ...
  const renderShop = () => (
    <View style={styles.shopContainer}>
      <Button mode="contained" style={styles.addButton} onPress={() => handleAddMerchInitiate()}>Add Merch</Button>
  ...
  const renderEvents = () => (
    <View style={styles.eventsContainer}>
      <Button mode="contained" style={styles.addButton} onPress={() => handleAddEventInitiate()}>Add Event</Button>
      {events.map(event => (
        <View key={event.id}>
          <EventCardAdmin
            eventId={event.id}
